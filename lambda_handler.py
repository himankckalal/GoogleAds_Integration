...
def get_resource_name(bearer_token,login_customer_id):
    # creating payload
    ...
    headers1.add("login-customer-id",  login_customer_id)
...
def generate_add_user_input_for_only_phone_numbers():
    ...
    s3.download_file('ci-360-data-dev-us-east-1','himank/Customer_Match.csv','/tmp/mycsv.csv')
...
def generate_add_users_input():
    ...
    with open('/tmp/mycsv.csv', 'r') as f:
        print(f.read())
        
        # csvFilePath = s3_location
    f = open( '/tmp/mycsv.csv', 'rU' )
        # Declare the column name from csv
...
def createOfflineUserDataJob(userlistResourceName,bearer_token,login_customer_id):
...
def addOperationsForUserData(offline_user_data_job_resource_name,bearer_token,login_customer_id):
...
def runOfflineJob(offline_user_data_job_resource_name,bearer_token,login_customer_id):
    # creating request header
    req_headers = {
    'developer-token': os.environ.get('developer_token'),
    'login-customer-id': login_customer_id,
...
def lambda_handler(event, context):
    ...
    login_customer_id = event["customer_id"]
    ...
    resource_name = get_resource_name(bearer_token,login_customer_id)
    logger.info('printing get_resource_name')
    logger.info(resource_name)
    # Creating offline_user_data_job_resource_name
    offline_user_data_job_resource_name = createOfflineUserDataJob(resource_name ,bearer_token,login_customer_id)
    ...
    addOperationsForUserData(offline_user_data_job_resource_name,bearer_token,login_customer_id)
    final_response = runOfflineJob(offline_user_data_job_resource_name,bearer_token,login_customer_id)
